...
def generate_zodiac_video(mode, target, date_str):
    ...
    if mode == 'compatibility':
        ...
        prompt = f"Viral compatibility script for {s1} and {s2}. Cover Love, Sex, and Trust."
    elif mode == 'birthday':
        prompt = f"Viral personality secrets for people born on {target}. Use Numerology."
        imgs = [] 
    else:
        imgs = [f"assets/zodiac_signs/{target}.jpg"]
        prompt = f"Viral {mode} prediction for {target} for {date_str}. Dark and mysterious tone."
